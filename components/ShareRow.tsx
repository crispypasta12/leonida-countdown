  ...
  return (
    <div className={`flex flex-wrap items-center justify-center gap-3 ${className}`}>
      <button
        type="button"
        onClick={handleShare}
        className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-pink to-violet
                   px-5 py-2.5 text-sm font-bold uppercase tracking-wide text-white
      ...
      <button
        type="button"
        onClick={handleCopy}
        className="inline-flex items-center gap-2 rounded-full border border-cyan/40 bg-white/5
                   px-4 py-2.5 text-sm font-semibold text-cyan transition hover:bg-white/10"
      >
        {copied ? (
          <>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
